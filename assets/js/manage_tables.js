function checkbox_click(event)
{
	...
	{
		$(event.target).parent().parent().find("td").addClass('selected').css("backgroundColor","blue");		
	}
	else
	{
...
function enable_delete(confirm_message,none_selected_message)
{
	...
	$('#delete').click(function(event)
	{
		...
		{       
                         ...
                         $('#delete-confirmation-msg').html(confirm_message);
                         
                         $('#delete-confirmation-yes').on('click', function() {
                              console.log($(link).attr('href'));
                              do_delete($(link).attr('href'));
                              $('#delete-confirmation').modal('hide');
                          });			
		}
		else
		{
                       $('#delete-not-selected').modal('show');
                       $('#delete-not-selected-msg').html(none_selected_message);
...
function do_delete(url)
{
	...
	$.post(url, { 'ids[]': row_ids },function(response)
	{
		...
		if(response.success)
		{
			...
			set_feedback(response.message,'alert alert-success',false);
                         $('#feedback_bar').show();
		}
		else
		{
			set_feedback(response.message,'alert alert-danger',true);
                        $('#feedback_bar').show();
		}
